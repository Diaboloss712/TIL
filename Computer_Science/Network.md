...
ARP : IP 주소를 통하여 MAC 주소를 찾는 프로토콜

IP 주소 : 클래스풀, 클래스리스 주소가 있다.

클래스풀 : 0, 10, 110 --> 128부터 2의 지수만큼 줄어든다고 계산하면 된다.

A, B, C, D, E 클래스가 있으며, 주로 A,B,C 클래스를 사용

클래스리스 : 서브넷 마스크를 통하여 세부적으로 주소를 나누는 방법

서브넷 마스크를 통하여 네트워크 주소를 알 수 있다.(& 연산자)

라우팅 : 최적의 경로로 설정하고 패킷을 보내는 것

라우팅에는 IGP, EGP 두 가지로 나뉜다.

IGP : RIP, OSPF(Open Shortest Path First)가 있으며,

RIP는 벡터 방식으로 홉이 짧은걸 계산하고

OSPF의 경우는 AS 단위로 나누고 중간에 ABR을 통하여 연결한다.

EGP : BGP가 있다. AS간 통신이 가능한 프로토콜이다.


## 4. 전송 계층

IP : 비연결형, 비신뢰성 통신으로 최선형 전달을 한다.

최선형 전달의 장점으로 전송되었음을 보장하지 않기에 속도가 빠르다.

포트 : 패킷의 최종 목적지는 프로세스로, 이런 프로세스를 식별할 수 있는 정보를 포트라고 한다.

NAT : 사설 -> 공인 변환 해주는 기술로 포트번호까지 하면 NAPT로 부른다.

사설 IP 주소 하나당 공인 IP 주소 하나가 대응

포트 포워딩은 특정 포트에 대한 연결을 내부로 전달할 수 있게 하는 방식이다.


TCP : 신뢰성, 연결성 - 체크섬, 확인 응답, 순서 번호, 제어 비트 등이 있다.

제어 비트에는 SYN, ACK, FIN이 있다.

무조건 수신되었는지 확인한다. (ARQ)

UDP : 비연결성으로 받을 때까지 던진다고 생각하면 된다.