...
시작하기에 앞서 이 글은 장고가 어떤 것이고, 어떤 기능이 있는지 코드를 보면서 최대한 따라갈 수 있게 적었다.

이 글의 목적 '이 상황에서 이런 흐름으로 가니까 다음으론 이게 필요하겠다'식으로

장고를 진행하는 흐름을 알아가면서 모르는 기능은 필요하다면 검색하거나 공식 문서를 통해 찾아보고 사용할 수 있다면 충분하다고 생각한다.
...
CSRF 토큰은 요청을 보낼때 해당 페이지에서 보낸 것이 맞는지 체크하는 토큰으로,

DB에 접근하는 요청일 때 해당 페이지에서 온 요청인지 검사할 때 사용한다.

만약 CSRF 토큰이 없다면 401 UnAuthorized가 발생하며,

이 경우 해당 페이지에서 CSRF 토큰을 부여하는지 체크해봐야 한다.